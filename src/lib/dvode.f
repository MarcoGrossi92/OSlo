...
C
C 21 March 2024 - Modification wrt the original file: MXSTP0 from 500 to 5000
C
C-----------------------------------------------------------------------
C Authors:
...
C              and is normally 5000.)  To continue, the user may
...
C     SUBROUTINE DEWSET_dvode (NEQ, ITOL, RTOL, ATOL, YCUR, EWT)
C where NEQ, ITOL, RTOL, and ATOL are as in the DVODE call sequence,
C YCUR contains the current dependent variable vector, and
C EWT is the array of weights set by DEWSET_dvode.
C
C If the user supplies this subroutine, it must return in EWT(i)
C (i = 1,...,NEQ) a positive quantity suitable for comparison with
C errors in Y(i).  The EWT array returned by DEWSET_dvode is passed to the
C DVNORM routine (See below.), and also used by DVODE in the computation
C of the optional output IMXER, the diagonal Jacobian approximation,
C and the increments for difference quotient Jacobians.
C
C In the user-supplied version of DEWSET_dvode, it may be desirable to use
C the current values of derivatives of y.  Derivatives up to order NQ
C are available from the history array YH, described above under
C Optional Output.  In DEWSET_dvode, YH is identical to the YCUR array,
C extended to NQ + 1 columns with a column length of NYH and scale
C factors of h**j/factorial(j).  On the first call for the problem,
C given by NST = 0, NQ is 1 and H is temporarily set to 1.0.
C NYH is the initial value of NEQ.  The quantities NQ, H, and NST
C can be obtained by including in DEWSET_dvode the statements:
...
C EWT is as set by subroutine DEWSET_dvode.
C
...
C            Replaced D1MACH by DUMACH_dvode.  Various changes to main
...
C  DEWSET_dvode    sets the error weight vector EWT before each step.
...
C  DUMACH_dvode    sets the unit roundoff of the machine.
C  XERRWD, XSETUN, XSETF, IXSAV, and IUMACH handle the printing of all
C            error messages and warnings.  XERRWD is machine-dependent.
C Note:  DVNORM, DUMACH_dvode, IXSAV, and IUMACH are function routines.
...
C
      DOUBLE PRECISION DUMACH_dvode, DVNORM
C
      DIMENSION MORD(2)
C-----------------------------------------------------------------------
...
C
      DATA  MORD(1) /12/, MORD(2) /5/, MXSTP0 /5000/, MXHNL0 /10/
...
C Load and invert the EWT array.  (H is temporarily set to 1.0.) -------
      NQ = 1
      H = ONE
      CALL DEWSET_dvode (N, ITOL, RTOL, ATOL, RWORK(LYH), RWORK(LEWT))
      DO 120 I = 1,N
...
C----------------------- End of Subroutine DVSRCO ----------------------
      END
*DECK DEWSET_dvode
      SUBROUTINE DEWSET_dvode (N, ITOL, RTOL, ATOL, YCUR, EWT)
C***BEGIN PROLOGUE  DEWSET_dvode
C***SUBSIDIARY
C***PURPOSE  Set error weight vector.
C***TYPE      DOUBLE PRECISION (SEWSET-S, DEWSET_dvode-D)
...
C
C***FIRST EXECUTABLE STATEMENT  DEWSET_dvode
 ...
 45     EWT(I) = RTOL(I)*ABS(YCUR(I)) + ATOL(I)
      RETURN
C----------------------- END OF SUBROUTINE DEWSET_dvode ----------------------
      END
*DECK DVNORM
...
C----------------------- End of Function IUMACH ------------------------
      END
*DECK DUMACH_dvode
      DOUBLE PRECISION FUNCTION DUMACH_dvode ()
C***BEGIN PROLOGUE  DUMACH_dvode
C***PURPOSE  Compute the unit roundoff of the machine.
C***CATEGORY  R1
C***TYPE      DOUBLE PRECISION (RUMACH-S, DUMACH_dvode-D)
C***KEYWORDS  MACHINE CONSTANTS
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C *Usage:
C        DOUBLE PRECISION  A, DUMACH_dvode
C        A = DUMACH_dvode()
C
C *Function Return Values:
...
C     number u such that  1.0 + u .ne. 1.0.  This is computed by DUMACH_dvode
C     in a machine-independent manner.
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  DUMSUM_dvode
C***REVISION HISTORY  (YYYYMMDD)
C   19930216  DATE WRITTEN
C   19930818  Added SLATEC-format prologue.  (FNF)
C   20030707  Added DUMSUM_dvode to force normal storage of COMP.  (ACH)
C***END PROLOGUE  DUMACH_dvode
C
      DOUBLE PRECISION U, COMP
C***FIRST EXECUTABLE STATEMENT  DUMACH_dvode
      U = 1.0D0
 10   U = U*0.5D0
      CALL DUMSUM_dvode(1.0D0, U, COMP)
      IF (COMP .NE. 1.0D0) GO TO 10
      DUMACH_dvode = U*2.0D0
      RETURN
C----------------------- End of Function DUMACH_dvode ------------------------
      END
      SUBROUTINE DUMSUM_dvode(A,B,C)
C     Routine to force normal storing of A + B, for DUMACH_dvode.
