...
CONTAINS
  ...
  SUBROUTINE INTEGRATE_RK( TIN, TOUT, N, NNZERO,VAR, RTOL, ATOL,FUN, JAC,&
    ...
    T1 = TIN; T2 = TOUT
    CALL RungeKutta(  N, NNZERO, T1, T2, VAR, RTOL, ATOL,FUN,JAC, &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE RungeKutta( N,NNZERO,T,Tend,Y,RelTol,AbsTol,FUN,JAC,  &
                         RCNTRL,ICNTRL,RSTATUS,ISTATUS,IERR )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
...
!    ICNTRL(1) = not used
!
!    ICNTRL(2) = 0: AbsTol, RelTol are NV-dimensional vectors
!              = 1: AbsTol, RelTol are scalars
!
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      INTEGER :: N,NNZERO
      DOUBLE PRECISION :: Y(N),AbsTol(N),RelTol(N),RCNTRL(20),RSTATUS(20)
      INTEGER :: ICNTRL(20), ISTATUS(20)
...
!~~~> Call the core method
      CALL RK_Integrator( N,T,Tend,Y,FUN,JAC,IERR )
...
!~~~> Arguments
      ...
      DOUBLE PRECISION, INTENT(INOUT) :: T, Y(NV)
      INTEGER,  INTENT(OUT)        :: IERR
      EXTERNAL ::FUN,JAC
!~~~> Local variables
      DOUBLE PRECISION, DIMENSION(NV) :: Z1,Z2,Z3,Z4,SCAL,DZ1,DZ2,DZ3,DZ4, &
      				G,TMP,F0
      DOUBLE PRECISION  :: CONT(NV,3), Tdirection,  H, Hacc, Hnew, Hold, Fac, &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      ErrOld = 1d-2
      Hacc = 1d-20
      CALL RK_ErrorScale(NV,ITOL,AbsTol,RelTol,Y,SCAL)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      ELSE
         ...
         CALL RK_Interpolate('eval',NV,H,Hold,Z1,Z2,Z3,CONT)
      END IF
...
NewtonLoop:DO  NewtonIter = 1, NewtonMaxit
            ...
            NewtonIncrement = SQRT( ( RK_ErrorNorm(NV,SCAL,DZ1)**2 + &
                                RK_ErrorNorm(NV,SCAL,DZ2)**2 + &
                                RK_ErrorNorm(NV,SCAL,DZ3)**2 )/3.0d0 )
            ...
            ! Update solution
            CALL DAXPY(NV,-ONE,DZ1,1,Z1,1) ! Z1 <- Z1 - DZ1
            CALL DAXPY(NV,-ONE,DZ2,1,Z2,1) ! Z2 <- Z2 - DZ2
            CALL DAXPY(NV,-ONE,DZ3,1,Z3,1) ! Z3 <- Z3 - DZ3
...
!       G = H*rkBgam(0)*F0 + rkTheta(1)*Z1 + rkTheta(2)*Z2 + rkTheta(3)*Z3
       CALL Set2Zero(NV, G)
       IF (rkMethod/=L3A) CALL DAXPY(NV,rkBgam(0)*H, F0,1,G,1)
       CALL DAXPY(NV,rkTheta(1),Z1,1,G,1)
       CALL DAXPY(NV,rkTheta(2),Z2,1,G,1)
       CALL DAXPY(NV,rkTheta(3),Z3,1,G,1)
...
!~~~>   Prepare the loop-dependent part of the right-hand side
	    CALL WADD(NV,Y,Z4,TMP)         ! TMP <- Y + Z4
	    CALL FUN(N,T+H,TMP,DZ4) 	  ! DZ4 <- Fun(Y+Z4)
            ISTATUS(Nfun) = ISTATUS(Nfun) + 1
!            DZ4(1:N) = (G(1:N)-Z4(1:N))*(rkGamma/H) + DZ4(1:N)
	    CALL DAXPY (NV, -ONE*rkGamma/H, Z4, 1, DZ4, 1)
        CALL DAXPY (NV, rkGamma/H, G,1, DZ4,1)

!~~~>   Solve the linear system
            CALL LSS_Solve(data,DZ4)
            ISTATUS(Nsol) = ISTATUS(Nsol) + 1
!~~~>   Check convergence of Newton iterations
            NewtonIncrement = RK_ErrorNorm(NV,SCAL,DZ4)
            IF ( NewtonIter == 1 ) THEN
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      IF (SdirkError) THEN
	 CALL Set2Zero(NV, DZ4)
         IF (rkMethod==L3A) THEN
           DZ4(1:NV) = H*rkF(0)*F0(1:NV)
	   IF (rkF(1) /= ZERO)  CALL DAXPY(NV, rkF(1), Z1, 1, DZ4, 1)
	   IF (rkF(2) /= ZERO)  CALL DAXPY(NV, rkF(2), Z2, 1, DZ4, 1)
	   IF (rkF(3) /= ZERO)  CALL DAXPY(NV, rkF(3), Z3, 1, DZ4, 1)
           TMP = Y + Z4
           CALL FUN(N,T+H,TMP,DZ1)
 	   CALL DAXPY(NV, H*rkBgam(4), DZ1, 1, DZ4, 1)
         ELSE
!         DZ4(1:N) =  rkD(1)*Z1 + rkD(2)*Z2 + rkD(3)*Z3 - Z4    
  	   IF (rkD(1) /= ZERO)  CALL DAXPY(NV, rkD(1), Z1, 1, DZ4, 1)
	   IF (rkD(2) /= ZERO)  CALL DAXPY(NV, rkD(2), Z2, 1, DZ4, 1)
	   IF (rkD(3) /= ZERO)  CALL DAXPY(NV, rkD(3), Z3, 1, DZ4, 1)
	   CALL DAXPY(NV, -ONE, Z4, 1, DZ4, 1)
         END IF
...
accept:IF (Err < ONE) THEN !~~~> STEP IS ACCEPTED
         ...
         IF (rkD(1) /= ZERO)  CALL DAXPY(NV,rkD(1),Z1,1,Y,1)
         IF (rkD(2) /= ZERO)  CALL DAXPY(NV,rkD(2),Z2,1,Y,1)
         IF (rkD(3) /= ZERO)  CALL DAXPY(NV,rkD(3),Z3,1,Y,1)
         ! Construct the solution quadratic interpolant Q(c_i) = Z_i, i=1:3
         IF (StartNewton) CALL RK_Interpolate('make',NV,H,Hold,Z1,Z2,Z3,CONT)
         CALL RK_ErrorScale(NV,ITOL,AbsTol,RelTol,Y,SCAL)
         RSTATUS(Ntexit) = T
         RSTATUS(Nhnew)  = Hnew
         RSTATUS(Nhacc)  = H
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      IF (rkMethod==L3A) THEN
         CALL DAXPY(N,-H*rkA(1,0),F0,1,R1,1) ! R1 <- R1 - h*A_10*F0
         CALL DAXPY(N,-H*rkA(2,0),F0,1,R2,1) ! R2 <- R2 - h*A_20*F0
         CALL DAXPY(N,-H*rkA(3,0),F0,1,R3,1) ! R3 <- R3 - h*A_30*F0
      END IF
...
!#else      
!      DOUBLE PRECISION    :: FJAC(LU_NONZERO),E1(LU_NONZERO)
!      COMPLEX(ki-nd=dp) :: E2(LU_NONZERO)
!#endif      
!      INTEGER :: IP1(N), IP2(N), i, j
      ...
      Alpha = rkAlpha/H
      Beta  = rkBeta /H
      CALL LSS_Decomp(data,Gamma,ISING)
      ...
      CALL LSS_Decomp_CMP(data,Alpha,Beta,ISING)     
      ISTATUS(Ndec) = ISTATUS(Ndec) + 1
...
!      
     ! Z <- h^{-1) T^{-1) A^{-1) x Z
      ...
      CALL LSS_Solve(data,R1)

      CALL LSS_Solve_CMP(data,R2,R3)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      INTEGER :: N
!#ifdef FULL_ALGEBRA      
!      DOUBLE PRECISION :: E1(N,N)
!#else      
!      DOUBLE PRECISION :: E1(LU_NONZERO)
!#endif      
